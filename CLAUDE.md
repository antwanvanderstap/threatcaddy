...
- **Database**: Dexie (IndexedDB). Schema: `src/db.ts`. Currently version 36.
- **Asset Management**: `src/components/Assets/` + `src/lib/asset-import.ts` + `src/lib/asset-correlation.ts` + `src/hooks/useAssets.ts`. Org-wide CMDB inventory (global, NOT investigation-scoped) imported from ITGlue/ConnectWise `configurations.csv` exports. Correlates SIEM event observables against inventory in three tiers: `exact` (MAC/serial/IP/hostname), `subnet` (/24 proximity), `gap` (internal address absent from CMDB). Assets link into investigations via `linkedFolderIds`. Non-technical CIs (licences, warranties, accounts) are filtered at import via `NON_TECHNICAL_TYPES`.
- **Asset edits**: analyst corrections live in an overlay (`Asset.overrides`, `src/lib/asset-overrides.ts`), never written over the imported values — a CMDB re-import replaces the base record wholesale, so in-place edits would be destroyed. **Always read through `resolveAsset`**; it is already applied inside `productsForAsset` and `buildAssetIndex`, so attack-surface and correlation pick corrections up automatically. `Asset.analystNotes` is analyst-owned and never written by an import.
- **Asset ownership (MSSP vs customer)**: `Asset.owner` (`mssp`/`customer`/`unknown`) + `Asset.customerName`, helpers in `src/lib/asset-ownership.ts`. Declared at import (a CMDB export has no organization column) or bulk-assigned from the filtered inventory view. Attack-surface exposures carry an `owners` rollup so "which customer runs this" is answerable.
- **Incident Response**: `src/lib/case-updates.ts` + `src/hooks/useCaseUpdates.ts` + `src/components/Investigation/CaseLogView.tsx`. Incidents are investigations (`Folder`) with incident fields — `severity`, `irPhase`, and the clock (`detectedAt`/`containedAt`/`eradicatedAt`/`recoveredAt`); `isIncident()` is true once severity is set to anything but `none`. Creation and playbooks already existed (`CreateInvestigationModal`, `PlaybookPicker`, `PlaybookExecution`) and were NOT rebuilt. `IncidentPhase` is the coarse lifecycle stage and is deliberately separate from the free-text `PlaybookStep.phase`. `caseUpdates` is a Dexie table keyed `[folderId+createdAt]`; edits keep superseded text in `revisions` so the log stays auditable. `advancePhase` backfills skipped milestones but never overwrites an existing stamp.
- **Attack Surface**: `src/lib/asset-products.ts` (normalize OS/hardware into CPE-shaped product identities) + `src/lib/asset-eol.ts` (curated lifecycle dataset, `EOL_DATASET_VERIFIED_AT` must be refreshed) + `src/lib/attack-surface.ts` (posture rollup + `assessApplicability` threat matching in `exact`/`product`/`keyword` tiers). UI: `AttackSurfaceTab`. Phase 1 (OS family + EOL) is complete; vendor-advisory and NVD/CVE phases reuse the same `ThreatDescriptor` matcher.
